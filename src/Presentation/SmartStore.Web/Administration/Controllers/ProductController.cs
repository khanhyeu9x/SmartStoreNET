...
namespace SmartStore.Admin.Controllers
{
    ...
    public partial class ProductController : AdminControllerBase
    {
        ...
        public ActionResult ProductPictureWatermark(int pictureId, int productId)
        {
            if (_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
            {
                var product = _productService.GetProductById(productId);
                ...
                var stream = _pictureService.OpenPictureStream(picture);
                if (pictureInfo != null)
                {
                    using (Image image = Image.FromStream(stream))
                    {
                        string myfile = picture.SeoFilename + "." + pictureInfo.Extension;
                        var pathExport = Server.MapPath("~/Content/ImgWatermark");
                        if (!Directory.Exists(pathExport))
                            Directory.CreateDirectory(pathExport);
                        var saveImagePath = Path.Combine(pathExport, myfile);
                        var store = Services.StoreService.GetAllStores().FirstOrDefault();

                        var logo = _pictureService.GetPictureById(store.LogoPictureId);
                        var streamLogo = _pictureService.OpenPictureStream(logo);
                        Image watermarkImage = Image.FromStream(streamLogo);
                        Watermarker objWatermarker = new Watermarker(image);

                        // Set the properties for the logo
                        objWatermarker.Position = WatermarkPosition.TopMiddle;
                        objWatermarker.ScaleRatio = 0.2f;
                        objWatermarker.Margin = new Padding(20);
                        objWatermarker.Opacity = 0.7f;
                        objWatermarker.TransparentColor = Color.White;
                        // Draw the logo
                        objWatermarker.DrawImage(watermarkImage);
                        // Draw QR Code

                        var productUrl = store.Url.EnsureEndsWith("/") + product.GetSeName();
                        QRCodeGenerator qrGenerator = new QRCodeGenerator();
                        QRCodeData qrCodeData = qrGenerator.CreateQrCode(productUrl + "?src=1", QRCodeGenerator.ECCLevel.Q);
                        QRCode qrCode = new QRCode(qrCodeData);
                        Bitmap qrCodeImage = qrCode.GetGraphic(20);
                        objWatermarker.Position = WatermarkPosition.BottomLeft;
                        objWatermarker.ScaleRatio = 0.2f;
                        objWatermarker.Margin = new Padding(0);
                        objWatermarker.TransparentColor = Color.Green;
                        objWatermarker.Opacity = 1f;
                        objWatermarker.DrawImage(qrCodeImage);
                        // Set the properties for the logo
                        objWatermarker.Position = WatermarkPosition.Absolute;
                        objWatermarker.PositionX = (int)((qrCodeImage.Width + 5) * 0.2f);
                        objWatermarker.PositionY = (int)(objWatermarker.Image.Height - 165);
                        objWatermarker.ScaleRatio = 0.15f;
                        objWatermarker.Opacity = 0.7f;
                        objWatermarker.TransparentColor = Color.White;
                        // Draw the logo
                        objWatermarker.DrawImage(watermarkImage);
                        ...
                        objWatermarker.Position = WatermarkPosition.Absolute;
                        objWatermarker.PositionX = (int)((qrCodeImage.Width + 5) * 0.2f);
                        objWatermarker.PositionY = (int)(objWatermarker.Image.Height - 90);
                        objWatermarker.ScaleRatio = 0.2f;
                        objWatermarker.TransparentColor = Color.White;
                        objWatermarker.Font = new Font(FontFamily.GenericSansSerif, 120f);
                        objWatermarker.FontColor = Color.Blue;
                        objWatermarker.DrawText("http://kmkids.vn");
                        //Draw the Text
                        objWatermarker.Position = WatermarkPosition.Absolute;
                        objWatermarker.PositionX = (int)((qrCodeImage.Width + 5) * 0.2f);
                        objWatermarker.PositionY = (int)(objWatermarker.Image.Height - 50);
                        objWatermarker.ScaleRatio = 0.2f;
                        objWatermarker.TransparentColor = Color.White;
                        objWatermarker.Font = new Font(FontFamily.GenericSansSerif, 138f);
                        objWatermarker.FontColor = Color.Blue;
                        objWatermarker.DrawText("0376.762.999");

                        objWatermarker.Image.Save(saveImagePath);
                        var imageResult = _imageProcessor.ProcessImage(new ProcessImageQuery(saveImagePath) {
                            Quality = _mediaSettings.DefaultImageQuality,
                            Format = pictureInfo.MimeType,
                            IsValidationMode = true
                        });
                        
                        return File(imageResult.OutputStream, pictureInfo.MimeType);
                    }
                }
            }
